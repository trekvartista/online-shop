...
const Basket = () => {
    ...
    useEffect(() => {
        ...
        fetchBasketItems( {basketId} ).then(data => basket.setBasket( data ))
        
    }, [])

    useEffect(() => {
        // console.log(basket.basket.rows)
        items.items.map( (item) => basket.basket.rows && basket.basket.rows.map( basket_item => basket_item.itemId === item.id && setBasketItems( [...basketItems, item] ) ) )
